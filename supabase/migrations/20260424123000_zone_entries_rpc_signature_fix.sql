-- Fix RPC overload ambiguity for get_zone_entries_for_team.
-- Keep:
--   - get_zone_entries_for_team()               -- participant/admin no-arg endpoint
--   - get_zone_entries_for_team(p_team_id UUID) -- explicit team endpoint (no default)
-- Remove default argument from UUID overload to avoid ambiguous resolution.

DROP FUNCTION IF EXISTS public.get_zone_entries_for_team();
DROP FUNCTION IF EXISTS public.get_zone_entries_for_team(UUID);
